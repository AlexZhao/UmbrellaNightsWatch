...
BPF_RINGBUF_OUTPUT(ring_dev_log, 16);

#define MAX_FILENAME_LEN 128
#define MAX_COMMNAME_LEN 64
// currently allow to record 10 nested folder
...
static __always_inline int ebpf_get_file_path(struct file *file, struct event *event) {
    ...
    if (file) {
        ...
        for (int i = 1; i < 10; i++) {
            ...
            if (bpf_probe_read_kernel(&new_parent, sizeof(new_parent), &parent->d_parent) != 0) {
                break;
            }
...
/// @brief LSM based access tracking
/// @param  
/// @param file 
/// @param cmd 
/// @param arg 
LSM_PROBE(file_ioctl, struct file *file, unsigned int cmd, unsigned long arg) {
    ...
    struct event *event = ring_dev_log.ringbuf_reserve(sizeof(struct event));
    if (!event) {
        return 0;
    }

    if (ebpf_get_task_app_name(task, event->comm, sizeof(event->comm)) != 0) {
        ring_dev_log.ringbuf_discard(event, 0);
        return 0;
    }
    ...
    event->file_op = FILE_OP_IOCTL;
    ring_dev_log.ringbuf_submit(event, 0);
